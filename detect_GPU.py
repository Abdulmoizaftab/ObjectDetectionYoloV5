...
with torch.no_grad():
            ...
            if(d!=''):
                for x in splitted:
                    ...
                    if 'Person' in x:
                        ...
                        DetectionClass.PistolOrGun=val
            time.sleep(2)
            t1 = threading.Thread(target=main1, args=(parse_opt1(),))
            t1.start()
            t1.join()
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            if(d != ''):
                for x in splitted:
                    ...
                    val = int(y[0])
                    if 'knife' in x:
                        DetectionClass.Knife=val
                    elif 'pistol' in x:
                        DetectionClass.PistolOrGun=val          
            time.sleep(2)
            t2 = threading.Thread(target=main2, args=(parse_opt2,))
            t2.start()
            t2.join()
    ...
    def parse_opt1():
        ...
        opt1.imgsz *= 2 if len(opt1.imgsz) == 1 else 1  # expand
        print_args(vars(opt1))
        return opt1
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            if(d!=''):
                for x in splitted:
                    ...
                    val = int(y[0])
                    if y[1] == 'Violence':
                        DetectionClass.Violence=val           
            time.sleep(2)
            t3 = threading.Thread(target=main3, args=(parse_opt3(),))
            t3.start()
            t3.join()
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            if(d!=''):
                for x in splitted:
                    ...
                    val = int(y[0])
                    if 'fire' in x:
                        DetectionClass.Fire=val
                    elif 'smoke' in x:
                        DetectionClass.Fire=val           
            time.sleep(2)
            t4 = threading.Thread(target=main4, args=(parse_opt4(),))
            t4.start()
            t4.join()
    ...
    def run4(
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                #                 print("fps are video",fps)
                #                 w = int(vid_cap.get(cv2.CAP_PROP_FRAME_WIDTH))
                #                 h = int(vid_cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
                #             else:  # stream
                #                 fps, w, h = 30, im0.shape[1], im0.shape[0]
                #                 print("fps are streaming",fps)
            ...
            cursor.execute("""Insert into Object_Detection (Did,ODetection,Person,Helmet,Mask,Knife,PistolORGun,Violence,Fire,Score,DateTimee) values ({},'{}',{},{},{},{},{},{},{},{},{});""".format(2,"Ali from GPU",DetectionClass.Person,DetectionClass.Helmet,DetectionClass.Mask,DetectionClass.Knife,DetectionClass.PistolOrGun,DetectionClass.Violence,DetectionClass.Fire,DetectionClass.Score,'CURRENT_TIMESTAMP'))
            cnxn.commit()
            allocc=torch.cuda.memory_allocated(0)
            print(allocc)
            torch.cuda.empty_cache()
            time.sleep(2)
            t5 = threading.Thread(target=main, args=(parse_opt(),))
            t5.start()
            t5.join()
